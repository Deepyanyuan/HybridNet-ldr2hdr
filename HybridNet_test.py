...
tf.flags.DEFINE_integer("learn_type", "0", "Learning type. (0:downexposure, 1:upexposure")
...
# load model
model, _ = network.HybridNet(x,x_local,is_train=False,batch_size=batch_size, pad='SAME')
yyy = model.outputs         
y = yyy*mask
...
tf.add_to_collection('pred_network',yyy)
...
for i in range(N):
    ...
    for input_frame_id in range(len(img_list)-1):
        ...
        loss_gen, loss_gen1, loss_gen2= sess.run([cost,cost1,cost2], feed_dict= {x: x_batch, x_local: x_batch, mask: lossmask, y_: y_batch})
